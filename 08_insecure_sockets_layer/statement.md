...
For the purposes of the `xorpos` and `addpos` operations, note that there is a **separate stream position counter** for the client-sent and server-sent streams, with each one starting at 0, and the counter is _not_ reset to 0 at the end of each request or response.
