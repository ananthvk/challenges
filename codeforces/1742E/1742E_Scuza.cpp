...
void solve(ll test_case_number)
{
    // Algorithm:
    // n -> number of steps
    // Let step_length[n] be the step sizes.
    // Create two arrays running_max[n] and prefix_sums[n]
    // and fill them with 0.
    // for i = 0 to n - 1
    //      if i != 0
    //          running_max[i] = max(running_max[i-1], step_length[i])
    //          prefix_sums[i] = prefix_sums[i-1] + step_length[i]
    //      else
    //          running_max[i] = running_max[i]
    //          prefix_sums[i] = step_length[i]

    // Use binary search to find the rightmost occurence of any step from running_max
    // This works becaue running_max is guaranteed to be in non decreasing order.
    // The corresponding value in the prefix_sums array gives the answer
    ll n = readn();
    ll q = readn();
    auto step_length = readvin(n);
    vl running_max(n, 0);
    vl prefix_sums(n, 0);
    FORN(i, n)
    {
    ...
    {
        ll query = readn();
        // Points to element which is greater than the target element
        auto it = upper_bound(running_max.begin(), running_max.end(), query);
        ll index = (it - running_max.begin() - 1);
        if (index < 0)
        {
            print(0, " ");
        }
