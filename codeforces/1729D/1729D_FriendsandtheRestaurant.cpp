...
void solve(ll test_case_number)
{
    ...
    vl diffarr;
    diffarr.reserve(n + 1);
    transform(x.begin(), x.end(), y.begin(), back_inserter(diffarr), [](ll a, ll b)
              { return b - a; });

    /*
    ll count_positive = 0;        // count of elements which are >= 0
    ll count_negative = 0;        // count of elements which are < 0
    ll min_negative = MINVAL(ll); // The smallest negative element (by magnitude)
    ll max_positive = MINVAL(ll); // The largest positive number
    for (const auto &e : diffarr)
    {
        if (e < 0)
        {
            ++count_negative;
            min_negative = max(e, min_negative); // -1 > -2, but magnitude wise, -1 is smaller
        }
        else
        {
            ++count_positive;
            max_positive = max(e, max_positive);
        }
    }
    ll answer = count_positive / 2;
    if (count_positive % 2 != 0 && count_negative > 0 && abs(min_negative) <= max_positive)
    {
        answer += 1;
    }
    */
