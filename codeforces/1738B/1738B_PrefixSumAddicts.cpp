...
{
    // Recreate the original array from the prefix sums and check if it is sorted
    ll n = readn();
    ll k = readn();
    auto a = readvin(k);
    vl real(k);
    ...
    if (!is_sorted(real.begin()+1, real.end()))
    {
        println("NO");
        return;
    }
    dprintln(a[0], " ", n - k + 1, " ", real[1]);
    dprintln(n - k + 1 * real[1]);
    dprintln(a[0]);
    if (a[0] > (n - k + 1) * real[1])
    {
        println("NO");
