...
result optimal(ll id, ll h, vector<ll> &t)
{
    sort(all(t));
    result res;
    ll cumul_sum = 0;
...
int main()
{
    ...
    FORN(nt, ntests)
    {
        ...
        {
            auto t = readvin(m);
            results[i] = optimal(i + 1, h, t);
            // println(results[i].id, " ", results[i].penalty, " ", results[i].points);
        }
        sort(all(results), [](const result &r, const result &l)
             {
            if(r.id == 1 && r.penalty == l.penalty)
                return true;
            return r.penalty < l.penalty; });
        stable_sort(all(results), [](const result &r, const result &l)
