...
int main()
{
    FAST_IO
    // Observations
    // The number of unique consecutive increasing sequeneces gives the answer
    // For example for 5 15 6 14 8 12 9 11
    // The dolls can be grouped as 5,6   8,9    11,12    14,15
    // Algorithm
    // ==========
    // Let count be a map storing the counts of each size of doll
    // num_sequences = 0
    // for each size of all sizes
    //      if count[size-1] <= 0
    //          num_sequences += 1
    //      else
    //          count[size-1] -= 1
