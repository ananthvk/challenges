...
{
    // If there is 1 in the array, the answer is NO
    ll n = readn();
    auto v = readvin(n);
    // sort(all(v));
    bool all_ones = true;
    bool no_ones = true;
    ll consecutives = false;
    sort(all(v));
    for (ll i = 0; i < n; ++i)
    {
        if (v[i] == 1)
    ...
    if (!no_ones && consecutives == 0)
    {
        // If there are no consecutive elements and there are some ones,
        // We can divide by max(a) - 1, to get all ones
        println("YES");
        return;
    }
    if (all_ones || no_ones)
