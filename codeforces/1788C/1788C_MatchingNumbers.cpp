...
int main()
{
    FAST_IO
    // Observations I made
    // 1. Always add 1 and 2n, this is the middle element of the series, let it be t
    // 2. There n/2 values on either side of n
    // 3. If n is even there is no solution
    // 4. For example: n = 7
    //           12 13 14 (15) 16 17 18
    // The question now is to find 2 numbers such that the sum = the value
    // Sometimes 2 or more pairs produce the same sum, and if we choose the wrong pair, another value will no longer have a pair
    // These are just my observations, they may or may not be true
    ll ntests = readn();
    FORN(nt, ntests)
    {
        ll n = readn();
        ...
        println("YES");
        // Better approach from https://www.youtube.com/watch?v=I4wF3a3xnZY
        ll other = 2 * n;
        for (ll i = 1; i <= n; i += 2)
        {
            println(i, " ", other);
