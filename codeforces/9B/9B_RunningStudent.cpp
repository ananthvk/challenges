...
int main()
{
    ...
    ll vb = readn();
    ll vs = readn();
    auto stops = readvin(n);
    ll uni_x = readn();
    ll uni_y = readn();
    vector<double> time_array;
    transform(stops.begin(), stops.end(), back_inserter(time_array), [=](ll stop_x)
              { return ((double)stop_x) / (double)(vb) + sqrt((stop_x - uni_x) * (stop_x - uni_x) + (uni_y * uni_y)) / vs; });
    ll index = min_element(time_array.begin(), time_array.end()) - time_array.begin();
    // println(time_array);
    double time = time_array[index];
    for (ll i = 0; const auto elem : time_array)
    {
        if (abs(elem - time) < 1e-4 && abs(stops[i] - uni_x) < abs(stops[index] - uni_x))
        {
            index = i;
        }
        ++i;
