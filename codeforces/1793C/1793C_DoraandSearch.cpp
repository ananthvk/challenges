...
int main()
{
    ...
    FORN(nt, ntests)
    {
        ll n = readn();
        auto v = readvin(n);
        // These conditions hold good only because v is a permutation
        // i.e. all numbers from 1 to n are present
        ll l = 0;
        ll r = n - 1;
        ll max_elem = n;
        ll min_elem = 1;
        while(l <= r){
            ...
            else if(v[l] == max_elem){
                ++l;
                --max_elem;
            }
            else if(v[r] == min_elem){
                --r;
                ++min_elem;
            }
            else if(v[r] == max_elem){
                --r;
                --max_elem;
            }
            else{
