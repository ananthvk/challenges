...
{
    FAST_IO
    // Solved it after looking at the hint
    // Divide the numbers into two sets, one for odd numbers and one for even numbers
    // For example:
    // -3 1 4 -1 5 -9
    // -3   4    5
    //    1   -1   -9
    // From each set find the maximum possible sum you get(only consider the positive numbers)
    ll nt = readn();
    FORN(nn, nt)
    {
        ll n = readn();
