...
{
    // Observations
    // 1. It is preferable to move digits with higher value than digits with lower value
    // 2. i.e. Perform operations on digits with higher value so that they
    // 3. End up at the end and at the same the digits with the lower value are pushed
    // 4. to the left

    // This is a O(10*n) solution
