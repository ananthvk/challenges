    ...
    FORN(nt, ntests)
    {
        ...
        for (ll i = 0; i < n; i++)
        {
            if (i != n - 1)
            {
                if (y[i + 1] >= (y[i] + h))
                {
                    areasum += (1.0 / 2.0) * ((double)d * (double)h);
                }
                else
                {
                    double trap_h = y[i] + h - y[i + 1];
                    // println("Trap:H:", trap_h);
                    //  println("a:", areasum);
                    areasum += (1.0 / 2.0) * (double)(d * h);
                    areasum -= (1.0/2.0) * ((d * trap_h) / ((double)(h))) * trap_h;
                }
            }
            else
            {
        ...
        std::cout<<std::setprecision(14)<<areasum<<"\n";
    }
    return 0;
}
