...
int main()
{
    FAST_IO
    ll n = readn();
    ll chess_r_min = MAXVAL(ll);
    ll chess_l_max = MINVAL(ll);
    FORN(i, n)
    {
        ll l = readn();
        ll r = readn();
        chess_r_min = min(r, chess_r_min);
        chess_l_max = max(l, chess_l_max);
    }
    ll m = readn();
    ll prog_l_max = MINVAL(ll);
    ll prog_r_min = MAXVAL(ll);
    FORN(i, m)
    {
        ll l = readn();
        ll r = readn();
        prog_l_max = max(l, prog_l_max);
        prog_r_min = min(r, prog_r_min);
    }
    println(max(max(prog_l_max - chess_r_min, chess_l_max - prog_r_min), (ll)0));
    return 0;
}
