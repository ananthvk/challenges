...
void solve(ll test_case_number)
{
    ...
    // Remove all elements which are in between two elements, i.e. a[i-1] < a[i] < a[i+1]
    // or a[i-1] > a[i] > a[i+1]
    // Since if a < b < c, a - b + c - b = a + c, i.e. the middle element can be removed
    vl a(b.begin(), it);
    ll sol = a.size();
    ...
    println(sol);
    // copy(a.begin(), it, ostream_iterator<ll>(std::cout, " "));
    // std::cout<<"\n";
}
