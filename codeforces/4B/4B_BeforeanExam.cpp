...
int main()
{
    ...
    ll d = readn();
    ll sumTime = readn();
    vector<ll> days_hrs(d, 0);
    vector<time_range> time_ranges(d);
    FORN(i, d)
    {
        time_ranges[i].minTime = readn();
        time_ranges[i].maxTime = readn();
        days_hrs[i] = time_ranges[i].minTime;
        sumTime -= time_ranges[i].minTime;
        if (sumTime < 0)
        {
            println("NO");
            return 0;
    ...
    FORN(i, d)
    {
        ll diff = time_ranges[i].maxTime - time_ranges[i].minTime;
        if (diff > sumTime)
        {
            days_hrs[i] += sumTime;
            sumTime = 0;
        ...
        {
            days_hrs[i] = time_ranges[i].maxTime;
            sumTime -= diff;
        }
    }
    if(sumTime > 0){
