...
void solve(ll test_case_number)
{
    ll n = readn();
    auto s = reads();
    // All substrings of size 1 are paranoid
    ll nparanoid = s.size();
    // for all other strings of size > 2, the string is paranoid only if s[m-1] != s[m]
    // abc01 has 01, c01, bc01, abc01, i.e. 4 substrings which are paranoid, so for a string of length m, there are m-1 substrings
    for (ll i = 1; i < n; ++i)
    {
        if (s[i] != s[i - 1])
        {
