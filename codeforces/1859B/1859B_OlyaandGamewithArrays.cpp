...
void solve(ll test_case_number)
{
    ...
    FORN(na, narrays)
    {
        ...
        auto a = readvin(n);
        sort(all(a));
        allmin = min(a[0], allmin);
        second_sums += a[1];
        ps.push_back({a[0], a[1]});
    }
    // Move all small elements to the one with smallest second element
    // Then the answer is min(all elements of all arrays) + sum of second mins of all arrays except this one
    // index: find the array with the smallest second minimum element
    ll index = min_element(ps.begin(), ps.end(), [](const p &l, const p &r)
                           { return l.second < r.second; }) -
               ps.begin();
    println(allmin + second_sums - ps[index].second);
}
