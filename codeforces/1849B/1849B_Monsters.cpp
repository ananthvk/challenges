...
void solve(ll ntest)
{
    ...
    FORN(i, n)
    {
        ...
        if (hp % k == 0)
        {
            monsters.push_back({i + 1, k});
        }
        else
        {
            monsters.push_back({i + 1, hp % k});
        }
    }
    // Ascending order of hp % k and incase of tie, by index
    sort(all(monsters), [=](const monster &l, const monster &r)
         {
        if(l.hp == r.hp){
...
void solve2(ll test_case_number)
{
    ll n = readn();
    ll k = readn();
    ...
    {
        ll hp = readn();
        monsters.push({i + 1, hp});
    }
    while (!monsters.empty())
    {
        auto top = monsters.top();
        monsters.pop();
        if ((top.hp - k) <= 0)
        {
            print(top.index, " ");
        ...
        {
            monsters.push({top.index, top.hp - k});
        }
    }
    println("");
}
