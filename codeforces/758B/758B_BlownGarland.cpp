...
int main()
{
    ...
    // c := integer array of length 4
    // legend := character array of length 26
    // for i = 0 to n - 1, increment by 4
    //      for j = 0 to 3
    //          if i + j is in bounds of array, a
    //              if a[i+j] == '!'
    //                  c[j] += 1
    //              else
    //                  legend[s[j+i] - 'A'] = j
    ...
    println(c[legend['R' - 'A']], " ", c[legend['B' - 'A']], " ", c[legend['Y' - 'A']], " ", c[legend['G' - 'A']]);
    return 0;
}
