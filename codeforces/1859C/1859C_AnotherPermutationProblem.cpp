...
{
    // The basic case is quite simple (when you want to find the sum, 1 <= i <= n, i.p_i)
    // Arrange the numbers in non decreasing order
    // The subtraction term complicates the problem, since we also have to subtract the max product we create
    // A bruteforce solution may work since n is quite small <= 250
