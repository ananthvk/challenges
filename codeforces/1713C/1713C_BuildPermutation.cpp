...
{
    ll n = readn();
    ll current_sqrt = (ll)sqrt(n) + 1;
    ll current_square = 1;
    ll i = n - 1;
    vector<ll> answer(n, -1);
    vector<ll> taken(n, 0);
    while (current_sqrt >= 0)
    {
        current_square = current_sqrt * current_sqrt;
        while ((current_square - i) <= (n - 1) && (i >= 0))
        {
            if (taken[current_square - i] || answer[i] != -1)
            {
                // Some other number is already here
                break;
            }
            answer[i] = current_square - i;
            taken[current_square - i] = 1;
            --i;
        }
        current_sqrt -= 1;
    }
