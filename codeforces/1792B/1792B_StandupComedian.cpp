...
int main()
{
    ...
    FORN(nt, n){
        ...
        ll t3 = readn();
        ll t4 = readn();
        // Observation from the editorial: If you take type2 and type3 in pairs, they never affect the result
        if(t1 != 0){
        ll total_told = 0;
        total_told += t1;
        total_told += min(t2, t3)*2;
        total_told += min(t1 + 1, abs(t3 - t2)+t4);
        println(total_told);
        }
        else{
            println(1);
