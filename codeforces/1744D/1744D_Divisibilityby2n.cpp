...
void solve(ll test_case_number)
{
    ll n = readn();
    auto a = readvin(n);
    // Find the total count of factors of 2 for each element in the array
    ll total_twos = 0;
    for (auto &n1 : a)
    {
        total_twos += pow2(n1);
    ...
    // Bruteforce solution, O(n*log(m) + nlogn), m a number
    // So I guess it should not TLE
    ll operations = 0;
    // Possible optimization: skip all odd numbers
    vl posops;
    posops.reserve(n);
    for (ll i = 1; i <= n; ++i)
    {
    ...
    sort(all(posops), greater<ll>());
    ll i = 0;
    while (total_twos < n && i < n)
    {
        total_twos += posops[i];
        if (posops[i] == 0)
            break;
