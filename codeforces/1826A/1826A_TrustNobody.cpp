...
void solve(ll test_case_number)
{
    ...
    // If there are n people in a group and one of them tells that there are atleast n liars in the group.
    // If the person is speaking the truth, then he is also one of the liars, so it is a contradiction.
    // If the person is a liar, it is possible.
    // All people who give n as the answer are liars.
    // Don't know if this assumption is correct
    ll n = readn();
    auto v = readvin(n);
    for (ll i = 0; i <= n; ++i)
    {
        // i represents the number of liers in the group (or the answer)
        // so all the people who say a number > i are liers
        ll liers1 = 0;
        for (const auto &e : v)
        {
            if (e > i)
