...
ll max_n(vl beg, ll i, ll k)
{
    ...
    return *max_element(beg.begin(), beg.end());
}
void solve(ll test_case_number)
{
    // Bruteforce solution, idk if it'll TLE or not
    ll n = readn();
    ll k = readn();
    auto v = readvin(n);
    ll answer = MINVAL(ll);
