...
int main()
{
    ...
    FORN(nn, nt)
    {
        ll n = readn();
        ll W = readn();
        // I guess there is a double rounding error when W ~ 10^18
        ll half_W = W - W / 2; // ceil(W/2.0);
        vector<weight> weights;
        FORN(i, n)
        {
            ll w = readn();
            weights.push_back({i + 1, w});
        }

        sort(all(weights), [](const weight &l, const weight &r)
        ...
        {
            if ((ww.weight + current_weight) <= W)
            {
                sol.push_back(ww.index);
                current_weight += ww.weight;
            }
        }
        if (current_weight >= half_W && current_weight <= W)
        {
            println(sol.size());
            println(sol);
        }
        ...
        // Three conditions
        // 1. Do not include weights which have a weight greater than W
        // 2. If there is a weight w such that ceil(W/2) <= w <= W, that is the answer
        // 3. Only add those weights which are less than ceil(W/2) to the list
        /*
        FORN(i, n)
        {
            ll w = readn();
        ...
        for(const auto& cw: weights){
            if(cw.weight + current_weight > W){
                break;
            }
            current_weight += cw.weight;
            sol.push_back(cw.index);
        }
        if(sol.empty() || current_weight < half_W){
            println(-1);
            continue;
        }
        println(sol.size());
        ...
        {
            if ((current_weight + we.weight) <= W)
            {
                current_weight += we.weight;
                sol.push_back(we.index);
                if (current_weight >= half_W)
                    break;
            }
        }
        if (current_weight == 0 || current_weight < half_W || sol.empty() || current_weight > W)
        {
            println(-1);
        }
        else
