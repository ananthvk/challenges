...
In particular, for example, the "long-tailed rat" and the "common long-tailed rat" are 2 _different_ species.

### Policies

A _policy_ is advice to the authority to either _conserve_ or _cull_ a particular _species_ at a particular _site_. Each created policy at a given site is identified by a unique `u32` "policy ID". The species is identified by a `str`.
...
This message is sent by your server to the Authority Server, to advise the authority to instate a new policy. The `species` field identifies the species, and the `action` field says whether the authority should _cull_ the species (value of `0x90`), or _conserve_ the species (value of `0xa0`).
...
It is an error for the `populations` field to contain multiple _conflicting_ counts for the same species (but non-conflicting duplicates are allowed).
